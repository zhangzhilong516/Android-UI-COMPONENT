...
import com.hengchang.ui_component.BuildConfig;
 ...
 */
public class UIPullRefreshLayout extends ViewGroup implements NestedScrollingParent {
    private static final String TAG = "UIPullRefreshLayout";
    ...
    public UIPullRefreshLayout(Context context) {
        this(context, null);
    }

    public UIPullRefreshLayout(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.UIPullRefreshLayoutStyle);
    }

    public UIPullRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTouchSlop = UIDisplayHelper.px2dp(context, mSystemTouchSlop); //系统的值是8dp,如何配置？
        ...
        TypedArray array = context.obtainStyledAttributes(attrs,
                R.styleable.UIPullRefreshLayout, defStyleAttr, 0);

        try {
            mRefreshInitOffset = array.getDimensionPixelSize(
                    R.styleable.UIPullRefreshLayout_ui_refresh_init_offset, Integer.MIN_VALUE);
            mRefreshEndOffset = array.getDimensionPixelSize(
                    R.styleable.UIPullRefreshLayout_ui_refresh_end_offset, Integer.MIN_VALUE);
            mTargetInitOffset = array.getDimensionPixelSize(
                    R.styleable.UIPullRefreshLayout_ui_target_init_offset, 0);
            mTargetRefreshOffset = array.getDimensionPixelSize(
                    R.styleable.UIPullRefreshLayout_ui_target_refresh_offset,
                    UIDisplayHelper.dp2px(getContext(), 72));
            mAutoCalculateRefreshInitOffset = mRefreshInitOffset == Integer.MIN_VALUE ||
                    array.getBoolean(R.styleable.UIPullRefreshLayout_ui_auto_calculate_refresh_init_offset, false);
            mAutoCalculateRefreshEndOffset = mRefreshEndOffset == Integer.MIN_VALUE ||
                    array.getBoolean(R.styleable.UIPullRefreshLayout_ui_auto_calculate_refresh_end_offset, false);
            mEqualTargetRefreshOffsetToRefreshViewHeight = array.getBoolean(R.styleable.UIPullRefreshLayout_ui_equal_target_refresh_offset_to_refresh_view_height, false);
        } finally {
            array.recycle();
        }
    ...
    private int moveTargetViewTo(int target, boolean isDragging, boolean calculateAnyWay) {
        ...
        if (target != mTargetCurrentOffset || calculateAnyWay) {
            ...
            if (mRefreshOffsetCalculator == null) {
                mRefreshOffsetCalculator = new UIDefaultRefreshOffsetCalculator();
            }
    ...
    public interface OnChildScrollUpCallback {
        boolean canChildScrollUp(UIPullRefreshLayout parent, @Nullable View child);
    }
    ...
    public static class RefreshView extends AppCompatImageView implements IRefreshView {
        ...
        public RefreshView(Context context) {
            ...
            setColorSchemeColors(UIResHelper.getAttrColor(context, R.attr.ui_app_main_color));
