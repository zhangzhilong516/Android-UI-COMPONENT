 ...
 */
public class UIRefreshRecyclerView extends UIWrapRecyclerView {
    // 下拉刷新的辅助类
    private UIRefreshViewCreator mRefreshCreator;
    // 下拉刷新头部的高度
    ...
    public UIRefreshRecyclerView(Context context) {
        super(context);
    }

    public UIRefreshRecyclerView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public UIRefreshRecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
    }

    // 先处理下拉刷新，同时考虑刷新列表的不同风格样式，确保这个项目还是下一个项目都能用
    // 所以我们不能直接添加View，需要利用辅助类
    public void addRefreshViewCreator(UIRefreshViewCreator refreshCreator) {
