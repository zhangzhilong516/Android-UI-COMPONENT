 ...
 */
public class UILoadRefreshRecyclerView extends UIRefreshRecyclerView {
    // 上拉加载更多的辅助类
    private UILoadViewCreator mLoadCreator;
    // 上拉加载更多头部的高度
    ...
    public UILoadRefreshRecyclerView(Context context) {
        super(context);
    }

    public UILoadRefreshRecyclerView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public UILoadRefreshRecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
    }

    // 先处理上拉加载更多，同时考虑加载列表的不同风格样式，确保这个项目还是下一个项目都能用
    // 所以我们不能直接添加View，需要利用辅助类
    public void addLoadViewCreator(UILoadViewCreator loadCreator) {
