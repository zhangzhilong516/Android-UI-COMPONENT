...
import com.hengchang.ui_component.alpha.UIAlphaImageButton;
import com.hengchang.ui_component.utils.UIDisplayHelper;
import com.hengchang.ui_component.utils.UIDrawableHelper;
...
/**
 * 通用的顶部 TitleBar。提供了以下功能：
 * <li>在左侧/右侧添加图片按钮/文字按钮/自定义 {@link View}。</li>
 * <li>设置标题/副标题，且支持设置标题/副标题的水平对齐方式。</li>
 * </ul>
 */
public class UITitleBarLayout extends RelativeLayout {
    ...
    public UITitleBarLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initVar();
        init(context, attrs, defStyleAttr);
        setId(R.id.ui_title_bar);
    }

    public UITitleBarLayout(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.UITitleBarStyle);
    }


    // ========================= centerView 相关的方法

    public UITitleBarLayout(Context context) {
        this(context, null);
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.UITitleBarLayout, defStyleAttr, 0);
        mTopBarSeparatorColor = array.getColor(R.styleable.UITitleBarLayout_ui_title_bar_separator_color,
                ContextCompat.getColor(context, R.color.ui_color_separator));
        mTopBarSeparatorHeight = array.getDimensionPixelSize(R.styleable.UITitleBarLayout_ui_title_bar_separator_height, 1);
        mTopBarBgColor = array.getColor(R.styleable.UITitleBarLayout_ui_title_bar_bg_color, Color.WHITE);
        getCommonFieldFormTypedArray(context, array);
        boolean hasSeparator = array.getBoolean(R.styleable.UITitleBarLayout_ui_title_bar_need_separator, true);
        array.recycle();
    ...
    void getCommonFieldFormTypedArray(Context context, TypedArray array) {
        mLeftBackDrawableRes = array.getResourceId(R.styleable.UITitleBarLayout_ui_title_bar_left_back_drawable_id, R.id.ui_title_bar_left_back);
        mTitleGravity = array.getInt(R.styleable.UITitleBarLayout_ui_title_bar_gravity, Gravity.CENTER);
        ...
        mTopBarImageBtnWidth = array.getDimensionPixelSize(R.styleable.UITitleBarLayout_ui_title_bar_image_btn_width, UIDisplayHelper.dp2px(context, 48));
        mTopBarImageBtnHeight = array.getDimensionPixelSize(R.styleable.UITitleBarLayout_ui_title_bar_image_btn_height, UIDisplayHelper.dp2px(context, 48));
        mTopBarTextBtnPaddingHor = array.getDimensionPixelSize(R.styleable.UITitleBarLayout_ui_title_bar_text_btn_padding_horizontal, UIDisplayHelper.dp2px(context, 12));
        mTopBarTextBtnTextColor = array.getColorStateList(R.styleable.UITitleBarLayout_ui_title_bar_text_btn_color_state_list);
        mTopBarTextBtnTextSize = array.getDimensionPixelSize(R.styleable.UITitleBarLayout_ui_title_bar_text_btn_text_size, UIDisplayHelper.sp2px(context, 16));
    ...
    private void updateTitleViewStyle() {
        if (mTitleView != null) {
            if (mSubTitleView == null || TextUtils.isEmpty(mSubTitleView.getText())) {
    ...
    private TextView getSubTitleView() {
        if (mSubTitleView == null) {
            ...
            LinearLayout.LayoutParams titleLp = generateTitleViewAndSubTitleViewLp();
            titleLp.topMargin = UIDisplayHelper.dp2px(getContext(), 1);
    ...
    private LayoutParams generateTitleContainerViewLp() {
        return new LayoutParams(LayoutParams.MATCH_PARENT,getTopBarHeight());
    }
    ...
    public UIAlphaImageButton addLeftBackImageButton() {
        return addLeftImageButton(mLeftBackDrawableRes, R.id.ui_title_bar_left_back);
    }
    ...
    private int getTopBarHeight() {
        if (mTopbarHeight == -1) {
            mTopbarHeight = UIResHelper.getAttrDimen(getContext(), R.attr.ui_title_bar_height);
        }
    ...
    public void setBackgroundDividerEnabled(boolean enabled) {
        if (enabled) {
            ...
            UIViewHelper.setBackgroundKeepingPadding(this, mTopBarBgWithSeparatorDrawableCache);
        } else {
            UIViewHelper.setBackgroundColorKeepPadding(this, mTopBarBgColor);
        }
    }
        ...
        if (mTitleContainerView != null) {
            ...
            if ((mTitleGravity & Gravity.HORIZONTAL_GRAVITY_MASK) == Gravity.CENTER_HORIZONTAL) {
                ...
                if (mLeftViewList.isEmpty()) {
                    //左侧没有按钮，标题离左侧间距
                    titleContainerViewLeft += UIResHelper.getAttrDimen(getContext(),
                            R.attr.ui_title_bar_margin_horizontal_when_no_btn_aside);
                }
            }
